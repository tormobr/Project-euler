...
def parse(g):
    grid = []
    for i,line in enumerate(g.splitlines()):
        grid.append([])
...
def vertical():
    best = grid[0][0] * grid[1][0] * grid[2][0] * grid[3][0]
    for i in range(20):
        for j in range(20-4):
            test = grid[j][i] * grid[j+1][i] * grid[j+2][i] * grid[j+3][i] 
            if test > best:
                best = test
...
def horizontal():
    best = grid[0][0] * grid[0][1] * grid[0][2] * grid[0][3]
    for row in grid:
        for j in range(1, 20-4):
            test = row[j] * row[j+1] * row[j+2] * row[j+3]
            if test > best:
                best = test
    return best

def diagonal_right():
    best = grid[0][0] * grid[1][1] * grid[2][2] * grid[3][3]
    for i in range(20-3):
        for j in range(20-3):
...
def diagonal_left():
    best = grid[19][19] * grid[18][18] * grid[17][17] * grid[16][16]
    for i in range(20-3):
...
results.append(diagonal_right())
results.append(diagonal_left())
print(max(results))
